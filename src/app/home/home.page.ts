...
export class HomePage {
  ...
  async pickFiles() {
    ...
    console.log(await Sevenzip.getDefaultPath())
  }
  // assets/data/sample-3.7z
  async startUnzip() {
    ...
    try {
      let cID = await Sevenzip.unzip(
        {
          // fileURL: this.currentSrc,
          fileURL: 'data/innova1.7z',
          outputDir: '/abc',
          rmSourceFile: false,
          isLocalAsset: true,
          // sqlLiteDBLocationConfig: 'Library/CapacitorDatabase'
        },
        (data: any) => {
  ...
  {
    console.log('CANCEL UNZIP-----------------------',await Sevenzip.cancelUnzipping());
  }
  async setDelayTime()
  {
    console.log('SET SLEEP TIME-----------------------',await Sevenzip.setSleepTime({
      sleepTime:1000
  }));
  }
}
